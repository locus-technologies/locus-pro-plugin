...
4. For live data, ask before a bounded pilot. Bind its source digest, inputs,
   destinations, budget, and expiry. Type checks and fixtures do not authorize
   a paid call.
5. Inspect results and receipts, then save the exact checked source as an
   immutable version. Editing a draft is not changing a saved version.
6. Start a production run only from a saved checked version with a hard
...
- `workflow_definition`: `create`, `update`, `get`, or `save`. Create and
  update accept `{files:[{path,content}]}` or a tenant-owned source
  `artifact_id`. Update requires `expected_revision`; save requires the exact
  revision's successful structural check.
...
- `workflow_run`: queues one immutable version in `pilot` or `run` mode. It
  ...
  dedicated `run_id`/status rather than blocking for the batch.
- `workflow_runs`: `get`, retrieve an `artifact` page, `cancel`, or `resume` an
  existing run. Get returns bounded durable events, call receipts, exact
  charges, and artifact metadata; artifact reads require both `run_id` and the
